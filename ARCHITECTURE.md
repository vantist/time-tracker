...
internal/
  ...
  transcript/    ExtractWindow()：從 JSONL transcript 擷取 token 用量（含 subagent）
...
Reconcile（補齊）
  → reconcile.MaybeReconcile()（tt serve 啟動時 / API 請求時）
      ├─ 找出 response_at 或 input_tokens 為 NULL 的 turns
      ├─ process.IsAlive() 確認 session 是否結束
      ├─ transcript.ExtractWindow() 從 JSONL 讀取 token
      └─ UPDATE turns（回填 response_at、tokens、cost）
```
...
turns    (id PK, session_id FK, prompt_at, response_at,
          ...
          transcript_path, prompt_line_offset)
```
