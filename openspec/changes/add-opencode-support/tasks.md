...
- [x] 6.2 整合測試：`tt setup --opencode` 產生合法 TS 檔，並以模擬的 `tt record prompt` + `tt record response --tokens --subagent-tokens` 呼叫驗證 `sessions` / `turns` / `turn_model_usages` 寫入正確（含 `is_subagent = 1` 明細）。
