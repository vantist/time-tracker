...
- [ ] 2.1 Write failing unit tests in `internal/report/report_test.go` to verify that `FormatText` prints all four categories (Input, Output, Cache read, Cache create) for all tables by default.
- [ ] 2.2 Update `FormatText` in `internal/report/report.go` to include cache token columns and display the 4-category breakdown for all tables. Verify the tests pass.
