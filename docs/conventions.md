...
3. **強健性與靜默處理 (Graceful Failure)**：
   - **Hook 靜默失敗**：Hook 程式執行出錯或連線失敗時，必須以 `exit 0` 退出，僅將錯誤訊息輸出至 `stderr`，絕對不能阻擋或中斷呼叫端（如 Claude Code/Copilot CLI）的正常開發工作。
   - **輸入優先級**：`stdin` 傳入的 JSON 優先於 CLI flag；flag 僅保留作為測試與手動執行時的備用手段。
   - **SQLite 資料庫**：使用純 Go 的 `modernc.org/sqlite`（無 CGO 依賴），確保跨平台交叉編譯（Cross-compilation）順暢。
   - **測試隔離**：測試時必須使用 `TT_DB_PATH` 環境變數指定獨立的測試資料庫檔案，避免干擾本機資料。
...
1. **PR 標題檢查 (`pr-lint.yml`)**：
   - 當 Pull Request 進行開立、編輯或同步時，會觸發 `amannn/action-semantic-pull-request@v5`。
   - 檢查 PR 標題是否符合 Conventional Commits 格式，若不符合則無法合併。

2. **自動釋出管理 (`release-please.yml`)**：
   - 當 Commit 被合併至 `main` 分支後，自動觸發並藉由 `googleapis/release-please-action@v4` 產生或更新 Release PR。
   - 若為 release-please 專屬的發布 PR，系統會藉由 GitHub CLI 自動啟用 `--auto --merge`，於檢查通過後自動完成合併。

3. **自動建置與釋出二進位檔 (`build.yml`)**：
   - 當推送 `v*` 格式的版號 tag 時觸發。
   ...
   - 建置指令使用 `-ldflags "-X main.version=$VERSION"` 將當前 tag 注入至二進位檔中。
   - 最終將二進位附件自動上傳至 GitHub Release。
